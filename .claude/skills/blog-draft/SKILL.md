...
- **질문형** ("~해도 돼?", "~이 뭐야?", "~하면 어떻게 돼?") → 질문의 주제를 추출해 그에 관한 블로그 초안을 작성한다
- **대화형** ("~을 써봤는데 어때?", "~에 대해 어떻게 생각해?") → 주제를 추출해 작성한다
- **단순 메모형** ("TypeScript satisfies 연산자") → 기존과 동일하게 작성한다

예시:
- "skill 안에 subagent를 만드는 것처럼 서로서로 안에다가 파일을 만들어도 돼?" → 주제: "Claude Code 스킬 디렉토리에 하위 파일·템플릿 구조 설계하기" → `programming/Tools` 카테고리 초안 작성
- "MCP 서버가 Claude Code에서 어떻게 동작해?" → 주제: "MCP 서버 동작 원리" → `programming` 카테고리 초안 작성
