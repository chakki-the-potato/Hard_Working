...
9. **커밋 + 푸시** — `draft({category}/{Tag1}): {글 제목}` 메시지로 main 브랜치에 직접 커밋한 뒤 즉시 `git push origin main` 실행
...
- `updatedDate`: 글 수정 시 추가하는 필드. 신규 초안에는 사용 안 함
...
- 스키마에 있는 optional 필드(`updatedDate`, `draft`, `heroImage`)도 명시 요청 없으면 추가하지 않음 — 위 "스키마 지원 optional 필드" 섹션 참조
...
- 스키마 지원 optional 필드를 임의로 추가 — `heroImage` / `draft` / `updatedDate`는 명시 요청이 있을 때만
- 기존에 발행된 글 수정 — 명시 요청 없으면 신규 초안만 작성. 기존 글 보강이 필요하면 사용자가 별도 지시
- 여러 글 동시 작성
- 글 작성 외 작업 — 코드 수정, 설정 파일 수정, 다른 스킬 수정, 워크플로우 수정 등 절대 금지 (이 스킬은 오직 src/data/blog/ 아래 마크다운 파일만 만든다)
