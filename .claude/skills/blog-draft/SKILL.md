...
8. **파일 저장** — `src/data/blog/{category}/{year}/{slug}.md`. 카테고리/연도 폴더가 없으면 새로 생성. 버전 업데이트 모드면 archive 파일도 같이 저장
9. **커밋 + 푸시** — 신규는 `draft({category}/{Tag1}): {글 제목}`, 갱신은 `update({category}/{Tag1}): {글 제목} → {new version}` 메시지로 main 브랜치에 직접 커밋한 뒤 즉시 `git push origin main` 실행
...
- `tags`가 비어 있으면 `draft({category}): {글 제목}` / `update({category}): {글 제목} → {new version}` 로 (괄호 유지)
