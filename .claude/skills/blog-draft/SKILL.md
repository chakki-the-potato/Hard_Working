...
메시지 형식 예시 (카테고리 다양성 포함):
- programming: "TypeScript의 satisfies 연산자, type assertion과 차이점 위주로"
- tools: "Claude Code 슬래시 커맨드 정리, 자주 쓰는 거 위주로"
...
2. 카테고리 결정 — `programming` / `thinking` / `design` / `tools` / `works` 중 하나 (소문자). 분류 기준은 아래 "카테고리 분류 가이드" 참조
...
- category: **필수 필드**. `programming` / `design` / `thinking` / `tools` / `works` 중 하나 (소문자). enum 미스매치 시 빌드 실패. 폴더 경로의 카테고리와 반드시 동일해야 함
- tags: 카테고리와 별개의 자유 라벨. 영문, 첫 글자 대문자, 0~3개. 예: `["Backend"]`, `["Build Tools", "Caching"]`. 비워도 OK

### works 카테고리 전용 optional 메타

`category: works`인 경우에만 의미가 있는 선택 필드. PostLayout이 이 값들을 메타 블록으로 렌더링한다. 다른 카테고리 글에서는 사용하지 않는다. 자동 라우팅으로 works가 결정된 경우에도 동일하게 적용 — 메시지에서 추출 가능한 정보를 모두 채운다.
...
자동 분류는 5종 전부 사용한다 (스키마 enum 전체).

- `programming`: 코드, 프레임워크, 라이브러리, 알고리즘, 시스템 설계, 언어 기능 등 "개념·구현"이 글의 중심
- `thinking`: 학습 방법, 커리어, 의사결정, 회고, 생각 정리, 책/강의 후기
- `design`: UI/UX, 타이포그래피, 색감, 디자인 시스템, 사용자 경험
- `tools`: 특정 명명된 도구의 "사용기·설정·워크플로우". CLI(Claude Code, gh, tmux 등), 에디터(neovim, VSCode, JetBrains 등), 플러그인·확장, 자동화 도구. 분류 기준은 *"도구 그 자체를 어떻게 쓰는가"*가 글의 중심인지. 도구 안에서 다루는 *개념·기법*이 중심이면 `programming`. 모호하면 `programming`으로 폴백
- `works`: 직접 만든 결과물 전용. 프로젝트·자체 제작 에이전트·공모전 출품작·사이드 프로젝트 런칭. 메시지에서 다음 신호 중 하나 이상이면 자동 라우팅:
  ...
  - works 글은 `src/data/blog/works/{year}/{slug}.md`에 저장하고, 위 "works 카테고리 전용 optional 메타" 필드를 가능한 범위에서 채움

> 사용자가 메시지에 "tools 카테고리로", "works 카테고리로" 같이 명시하면 위 자동 판단을 무시하고 명시한 카테고리를 사용한다.

#### 분류 예시 (borderline 포함)

| 메시지 | 카테고리 | 이유 |
|---|---|---|
| "TypeScript satisfies 연산자" | `programming` | 언어 기능 자체 |
| "Docker 멀티스테이지 빌드" | `programming` | 빌드 *개념*. 도구 사용기가 아님 |
| "Claude Code 슬래시 커맨드 정리" | `tools` | 특정 도구 사용기 |
| "neovim LSP 설정" | `tools` | 에디터 설정 |
| "tmux 워크플로우" | `tools` | 도구 워크플로우 |
| "VSCode에서 useEffect 무한 루프 디버깅" | `programming` | 도구 안에서 다룬 *개념·기법*이 중심 |
| "gh CLI로 PR 자동화" | `tools` | gh 자체 사용법이 중심 |
