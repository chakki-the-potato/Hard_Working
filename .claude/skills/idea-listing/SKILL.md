...
## 원칙: 모든 메시지는 콘텐츠 생성 요청이다

텔레그램으로 오는 **모든 메시지**는 콘텐츠 생성 요청이다. 사용자에게 질문에 대답하거나 대화를 나누는 것은 이 스킬의 역할이 아니다.

- **질문형** ("~해도 돼?", "~이 뭐야?", "~하면 어떻게 돼?") → 질문의 주제를 추출해 그에 관한 아이디어 파일을 작성한다
- **대화형** ("~을 써봤는데 어때?", "~에 대해 어떻게 생각해?") → 주제를 추출해 작성한다
- **단순 메모형** ("TypeScript satisfies 연산자") → 기존과 동일하게 작성한다

예시:
- "skill 안에 subagent를 만드는 것처럼 서로서로 안에다가 파일을 만들어도 돼?" → 주제: "Claude Code 스킬 디렉토리에 하위 파일·템플릿 구조 설계하기" → `programming/Tools` 카테고리 아이디어 파일 작성
- "MCP 서버가 Claude Code에서 어떻게 동작해?" → 주제: "MCP 서버 동작 원리" → `programming` 카테고리 아이디어 파일 작성
