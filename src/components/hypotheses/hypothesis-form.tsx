  ...
  const statusOptions = mode === "create"
    ...
    : initialValues.status === "concluded" || initialValues.status === "abandoned"
      ...
      : initialValues.status === "running"
        ? ["running", "concluded", "abandoned"] as const
  ...
  return (
    <form action={formAction} className="admin-editor hypothesis-form">
      ...
      <div className="admin-editor-grid">
        ...
        <label className="admin-field"><span className="admin-field-label">공개 요약</span><textarea aria-describedby={state.fieldErrors.publicSummary ? "hypothesis-summary-error" : undefined} className="admin-textarea" defaultValue={state.values.publicSummary} maxLength={1000} name="publicSummary" rows={3} />{state.fieldErrors.publicSummary ? <span className="admin-field-error" id="hypothesis-summary-error">{state.fieldErrors.publicSummary}</span> : null}</label>
